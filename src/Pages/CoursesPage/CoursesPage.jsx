...
export default function CoursesPage() {
    ...
    const getOneCourse = useCallback(() => {
        fetch(`https://educational-web-site.vercel.app/v1/courses/${id}`, {
            method: 'GET',
            headers: {
    ...
    useEffect(() => {
        fetch(`https://educational-web-site.vercel.app/v1/courses/related/${id}`)
            .then(res => res.json())
            .then(result => {
    ...
    const sendComment = async (newComment) => {
        ...
        await fetch(`https://educational-web-site.vercel.app/v1/comments`, {
            method: "POST",
            headers: {
    ...
    const registerInCourseConfirmation = () => {
        if (oneCourse.price == 0) {
            fetch(`https://educational-web-site.vercel.app/v1/courses/${oneCourse?._id}/register`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorageData.token}`
    ...
    useEffect(() => {
        if (confirm) {
            fetch(`https://educational-web-site.vercel.app/v1/offs/${off}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorageData.token}`
                    ...
                    <div className="slide mx-auto mt-8 sm:mt-10">
                        ...
                        <div className="rounded-2xl flex flex-col lg:flex-row-reverse lg:justify-between gap-y-4 lg:p-0 p-4 mt-8 sm:mt-10 lg:gap-x-6 lg:!bg-transparent bg-[#f3f4f6] dark:bg-[#242A38] items-center xl:items-start">
                            <div className="lg:w-1/2">
                                <video className="rounded-2xl" controls poster={`https://educational-web-site.vercel.app/courses/covers/${oneCourse.cover}`}>
                                    <source />
                                </video>
                            </div>
                        ...
                        <div className="grid grid-cols-12 lg:mt-[70px] gap-8 mt-5">
                            <div className="col-span-12 lg:col-span-8">
                                ...
                                <BoxesTitle bg={"bg-amber-500"} title={"توضیحات"} icon={<IoDocumentText className="hidden md:inline-block text-amber-400 w-10 h-10" />}>
                                    ...
                                    <img className="rounded-2xl" src={`https://educational-web-site.vercel.app/courses/covers/${oneCourse.cover}`} alt="" />
                                            ...
                                            {relatedCourses.map(relatedCourses => (
                                                ...
                                                <div key={relatedCourses._id} className="flex items-center justify-between flex-wrap bg-[#f3f4f6] dark:bg-[#333C4C] rounded-lg py-2 pr-2 pl-4">
                                                    <div className="flex items-center gap-x-4 w-4/5">
                                                        <img className="w-24 rounded-md aspect-video" src={`https://educational-web-site.vercel.app/courses/covers/${relatedCourses.cover}`} alt={relatedCourses.name} />
